package admin; // O el paquete donde quieras ponerlo
...
import DAO.PlagaDAO; // DAO Principal
import Modelos.Plaga; // Modelo Principal
import java.awt.event.WindowAdapter;
import java.awt.event.WindowEvent;
import java.sql.SQLException;
import java.sql.Connection;

public class plagaAd extends JFrame {
    private JTable membersTable;
    private DefaultTableModel tableModel;
    
    // Campos del formulario
    private JTextField nombrePlagaField, especieField;
    ...
    public plagaAd(JFrame parentMenu_recibido, Connection conn_recibida) throws SQLException {
    ...
    private void crearPanelSuperior() {
        ...
        JLabel title = new JLabel("Plagas");
        title.setFont(new Font("Segoe UI", Font.BOLD, 36));
        title.setBounds(40, 15, 500, 45);
        title.setForeground(new Color(30, 100, 35));
        topPanel.add(title);

        JLabel subtitle = new JLabel("Gestión de plagas y especies");
    ...
    private void crearPanelFormulario() {
        ...
        JLabel formTitle = new JLabel("Gestión de Plaga");
        ...
        // --- Campos del formulario ---
        int yPos = 60;
        nombrePlagaField = crearCampoFormulario(leftPanel, "Nombre de la Plaga*", yPos);
        yPos += 70;
        ...
        addBtn.addActionListener(e -> agregarPlaga());
        updateBtn.addActionListener(e -> actualizarPlaga());
    ...
    private void crearPanelTabla() {
        ...
        JLabel tableTitle = new JLabel("Lista de Plagas");
        ...
        // Acción: Recargar datos de PLAGAS
        refreshBtn.addActionListener(e -> {
            cargarPlagas(); // Recarga la tabla
        });
        ...
        String[] columnNames = {"ID", "Nombre Plaga", "Especie", "Acciones"};
        tableModel = new DefaultTableModel(columnNames, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return column == 3; // Columna de acciones
            }
            @Override
            public Class<?> getColumnClass(int column) {
                if (column == 3) return JPanel.class;
                return Object.class;
            }
        };
        ...
        membersTable.getColumnModel().getColumn(1).setPreferredWidth(200); // Nombre
        membersTable.getColumnModel().getColumn(2).setPreferredWidth(200); // Especie
        membersTable.getColumnModel().getColumn(3).setPreferredWidth(130); // Acciones
        membersTable.getColumnModel().getColumn(3).setMaxWidth(150);
        ...
        // Asignar renderer/editor a la columna 3 (Acciones)
        membersTable.getColumnModel().getColumn(3).setCellRenderer(new AccionesRenderer());
        membersTable.getColumnModel().getColumn(3).setCellEditor(new AccionesEditor());
    ...
    private class AccionesEditor extends DefaultCellEditor {
        ...
        public Component getTableCellEditorComponent(JTable table, Object value,
            ...
            editarBtn.addActionListener(e -> {
                ...
                editarPlagaDesdeBoton(id);
            });

            eliminarBtn.addActionListener(e -> {
                fireEditingStopped();
                int id = (int) tableModel.getValueAt(row, 0);
                eliminarPlagaDesdeBoton(id);
            });
    ...
    private void cargarPlagas() {
        ...
        for (Plaga plaga : plagas) {
            ...
            tableModel.addRow(new Object[]{
                plaga.getIdPlaga(),     // Col 0 (Oculta)
                plaga.getNombrePlaga(), // Col 1
                plaga.getEspecie(),     // Col 2
                panelAcciones           // Col 3 (Panel de Acciones)
            });
        }
        totalLabel.setText("Total: " + plagas.size());
    }

    private void editarPlagaDesdeBoton(int id) {
        Plaga plaga = plagaDAO.obtenerPlagaPorId(id);
        if (plaga != null) {
            cargarPlagaEnFormulario(plaga);
        }
    }

    private void eliminarPlagaDesdeBoton(int id) {
        Plaga plaga = plagaDAO.obtenerPlagaPorId(id);
        if (plaga != null) {
            int confirm = JOptionPane.showConfirmDialog(this,
                    "¿Está seguro de eliminar la plaga: " + plaga.getNombrePlaga() + "?",
                    "Confirmar Eliminación", JOptionPane.YES_NO_OPTION);

            if (confirm == JOptionPane.YES_OPTION) {
                if (plagaDAO.eliminarPlaga(id)) {
                    JOptionPane.showMessageDialog(this, "Plaga eliminada correctamente", "Éxito", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void cargarPlagaEnFormulario(Plaga plaga) {
        idActual = plaga.getIdPlaga();
        nombrePlagaField.setText(plaga.getNombrePlaga());
        especieField.setText(plaga.getEspecie());
    }

    private void agregarPlaga() {
        Plaga plaga = obtenerPlagaDesdeFormulario(false);
        if (plaga == null) return;
        
        if (plagaDAO.existePlagaPorNombre(plaga.getNombrePlaga())) {
            JOptionPane.showMessageDialog(this, "Ya existe una plaga con ese nombre", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        if (plagaDAO.agregarPlaga(plaga)) {
            JOptionPane.showMessageDialog(this, "Plaga agregada correctamente", "Éxito", JOptionPane.INFORMATION_MESSAGE);
            cargarPlagas();
            limpiarFormulario();
        }
    }
    private void actualizarPlaga() {
        if (idActual == 0) {
            JOptionPane.showMessageDialog(this, "Seleccione una plaga para editar (clic en 'Editar' en la tabla)", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        Plaga plaga = obtenerPlagaDesdeFormulario(true);
        if (plaga == null) return;
        
        plaga.setIdPlaga(idActual);
        
        Plaga existente = plagaDAO.obtenerPlagaPorNombre(plaga.getNombrePlaga());
        if (existente != null && existente.getIdPlaga() != plaga.getIdPlaga()) {
            JOptionPane.showMessageDialog(this, "Ya existe otra plaga con ese nombre", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        if (plagaDAO.actualizarPlaga(plaga)) {
            JOptionPane.showMessageDialog(this, "Plaga actualizada correctamente", "Éxito", JOptionPane.INFORMATION_MESSAGE);
            cargarPlagas();
            limpiarFormulario();
        }
    }

    private Plaga obtenerPlagaDesdeFormulario(boolean esActualizacion) {
        String nombre = nombrePlagaField.getText().trim();
        ...
        return new Plaga(0, nombre, especie);
    }

    private void limpiarFormulario() {
        idActual = 0;
        nombrePlagaField.setText("");
