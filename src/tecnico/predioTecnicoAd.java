...
public class predioTecnicoAd extends JFrame {
    ...
    private JTextField txtUbicacion, txtHectareas, txtLugar;

    private Connection conn;
    private JFrame parentMenu;
    private PredioDAO predioDAO;

    public predioTecnicoAd(JFrame parentMenu, Connection conn) {
        this.parentMenu = parentMenu;
        this.conn = conn;
        this.predioDAO = new PredioDAO(conn);
    ...
    private void initializeUI() {
        ...
        JLabel title = new JLabel("Predios del Sistema");
        ...
        JLabel l1 = new JLabel("Detalle del Predio");
        l1.setFont(new Font("Segoe UI", Font.BOLD, 20));
        l1.setForeground(PRIMARY_COLOR);
        l1.setBounds(20, 20, 300, 30);
        left.add(l1);

        crearLabel(left, "Ubicación:", 80);
        txtUbicacion = crearInput(left, 105);
        
        crearLabel(left, "Hectáreas:", 160);
        txtHectareas = crearInput(left, 185);
        
        crearLabel(left, "ID Lugar Producción:", 240);
        txtLugar = crearInput(left, 265);
        ...
        tableModel = new DefaultTableModel(new String[]{"ID", "Ubicación", "Hectáreas", "ID Lugar"}, 0){
    ...
    private void cargarDatos() {
        ...
        for(Predio p : predioDAO.obtenerTodosPredios()) {
            tableModel.addRow(new Object[]{p.getIdPredio(), p.getUbicacionMun(), p.getExtensionHectareas(), p.getIdLugarProduccion()});
        }
    }
    ...
    private void mostrarDetalle() {
        ...
        if(row != -1) {
            txtUbicacion.setText(table.getValueAt(row, 1).toString());
            txtHectareas.setText(table.getValueAt(row, 2).toString());
