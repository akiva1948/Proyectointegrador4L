...
public class inspeccionTecnicoAd extends JFrame {
    ...
    private JTextField txtCantidad, txtAfectadas;
    private JComboBox<Lote> loteCombo;
    private JComboBox<Plaga> plagaCombo;
    // NOTA: No hay combo de Técnico porque es el usuario logueado
    ...
    public inspeccionTecnicoAd(JFrame parentMenu_recibido, Connection conn_recibida, String correoTecnico) {
        ...
        try {
            ...
            // 1. Obtener ID del Técnico Logueado
            this.idTecnicoLogueado = tecnicoDAO.obtenerIdPorCorreo(correoTecnico);
            
            if (this.idTecnicoLogueado == -1) {
                JOptionPane.showMessageDialog(this, "Error crítico: No se pudo identificar al técnico.", "Error de Sesión", JOptionPane.ERROR_MESSAGE);
                this.dispose();
                return;
            }
    ...
    private void initializeUI() {
        setTitle("Panel Técnico - Gestión de Inspecciones");
        setSize(1366, 800);
    ...
    private void crearPanelSuperior() {
        ...
        JLabel title = new JLabel("Mis Inspecciones Fitosanitarias");
        title.setFont(new Font("Segoe UI", Font.BOLD, 36));
        title.setBounds(40, 15, 600, 45);
        title.setForeground(new Color(30, 100, 35));
        topPanel.add(title);

        JLabel subtitle = new JLabel("Técnico: " + correoTecnico);
    ...
    private void crearPanelFormulario() {
        ...
        JLabel formTitle = new JLabel("Registrar Nueva Visita");
        ...
        // 2. Combos (Sin Técnico)
        loteCombo = crearComboFormulario(leftPanel, "Lote a Revisar*", yPos); yPos += 65;
        ...
        obsScrollPane.setBounds(0, yPos + 25, 300, 60);
        ...
        JButton updateBtn = crearBotonEstilizado("Corregir", new Color(80, 160, 85));
    ...
    private void crearPanelTabla() {
        ...
        JLabel tableTitle = new JLabel("Historial de Inspecciones");
        ...
        // Columnas (Igual a Admin, pero sin columna Técnico porque ya sabemos quien es)
        String[] columnNames = {"ID", "Fecha", "Lote", "Plaga", "Incidencia", "Alerta", "Acciones"};
        tableModel = new DefaultTableModel(columnNames, 0) {
            @Override public boolean isCellEditable(int row, int column) { return column == 6; } // Solo columna acciones
        ...
        membersTable.getColumnModel().getColumn(6).setPreferredWidth(100); // Acciones más pequeño (sin botón X)
        ...
        // Leyenda Footer
        JLabel info = new JLabel(" * Alerta e Incidencia calculadas automáticamente por el sistema. No puede eliminar registros.");
        info.setFont(new Font("Segoe UI", Font.ITALIC, 11));
        info.setForeground(Color.GRAY);
        rightPanel.add(info, BorderLayout.SOUTH);
    }

    // --- Renderers y Editores (MODIFICADO: Sin botón Eliminar) ---
    ...
    private class AccionesEditor extends DefaultCellEditor {
        ...
        public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
            ...
            panel.add(editarBtn);
            // NOTA: Botón Eliminar REMOVIDO para el técnico
            return panel;
        }
        public Object getCellEditorValue() { return this.currentValue; }
    }

    // --- Helpers (Idénticos a Admin) ---
    ...
    private void cargarInspecciones() {
        ...
        for (Inspeccion insp : inspecciones) {
            ...
            panelAcciones.add(new JButton("Editar")); // Visual only
    ...
    private void editarInspeccionDesdeBoton(int id) {
        ...
        if (inspeccion != null) {
            // Verificación de seguridad (Opcional: Solo editar las propias)
            // if (inspeccion.getIdTecnico() != idTecnicoLogueado) {
            //     JOptionPane.showMessageDialog(this, "Solo puedes editar tus propias inspecciones.");
            //     return;
            // }
            ...
            txtCantidad.setText(String.valueOf(inspeccion.getCantidadPlantas()));
            txtAfectadas.setText(String.valueOf(inspeccion.getPlantasAfectadas()));
            
            // Seleccionar Lote
            for (int i = 0; i < loteCombo.getItemCount(); i++) {
                if (loteCombo.getItemAt(i).getIdLote() == inspeccion.getIdLote()) {
                    loteCombo.setSelectedIndex(i); break;
                }
            }
            // Seleccionar Plaga
            for (int i = 0; i < plagaCombo.getItemCount(); i++) {
                if (plagaCombo.getItemAt(i).getIdPlaga() == inspeccion.getIdPlaga()) {
                    plagaCombo.setSelectedIndex(i); break;
                }
            }
        }
    }

    // NOTA: Función eliminar eliminada
    ...
    private void agregarInspeccion() {
        ...
        if (inspeccionDAO.agregarInspeccion(insp)) {
            JOptionPane.showMessageDialog(this, "Inspección registrada. Alerta calculada.");
    ...
    private void actualizarInspeccion() {
        ...
        if (inspeccionDAO.actualizarInspeccion(insp)) {
            JOptionPane.showMessageDialog(this, "Registro corregido.");
            cargarInspecciones(); limpiarFormulario();
        }
    }
    ...
    private Inspeccion obtenerInspeccionDesdeFormulario() {
        ...
        if (fecha == null || obs.isEmpty() || sCant.isEmpty() || sAfec.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Faltan datos obligatorios"); return null;
        }
        ...
        try {
            ...
            if (afec > cant) {
                JOptionPane.showMessageDialog(this, "Error: Plantas afectadas > Total plantas"); return null;
            }
        } catch(Exception e) {
             JOptionPane.showMessageDialog(this, "Cantidades deben ser números enteros"); return null;
        }
        
        Lote l = (Lote) loteCombo.getSelectedItem();
        Plaga p = (Plaga) plagaCombo.getSelectedItem();
        
        // ¡AQUI ESTÁ LA MAGIA! Usamos idTecnicoLogueado automáticamente
        return new Inspeccion(0, fecha, obs, l.getIdLote(), idTecnicoLogueado, p.getIdPlaga(), cant, afec);
    }
