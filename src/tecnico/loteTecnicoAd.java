...
public class loteTecnicoAd extends JFrame {
    ...
    private JTextField txtFecha, txtEstado, txtLugar, txtCultivo;

    private Connection conn;
    private JFrame parentMenu;
    private LoteDAO loteDAO;

    public loteTecnicoAd(JFrame parentMenu, Connection conn) {
    ...
    private void initializeUI() {
        ...
        JLabel title = new JLabel("Lotes y Cultivos");
        ...
        JLabel l1 = new JLabel("Detalle del Lote");
        l1.setFont(new Font("Segoe UI", Font.BOLD, 20));
        l1.setForeground(PRIMARY_COLOR);
        l1.setBounds(20, 20, 300, 30);
        left.add(l1);

        crearLabel(left, "Fecha Siembra:", 80);
        txtFecha = crearInput(left, 105);
        
        crearLabel(left, "Estado:", 160);
        txtEstado = crearInput(left, 185);
        
        crearLabel(left, "ID Cultivo:", 240);
        txtCultivo = crearInput(left, 265);
        
        crearLabel(left, "ID Lugar:", 320);
        txtLugar = crearInput(left, 345);
        ...
        tableModel = new DefaultTableModel(new String[]{"ID", "Fecha", "Estado", "ID Cultivo", "ID Lugar"}, 0){
    ...
    private void cargarDatos() {
        ...
        for(Lote l : loteDAO.obtenerTodosLotes()) {
            tableModel.addRow(new Object[]{
                l.getIdLote(), 
                sdf.format(l.getFechaSiembra()), 
                l.getEstado(), 
                l.getIdCultivo(), 
    ...
    private void mostrarDetalle() {
        ...
        if(row != -1) {
            txtFecha.setText(table.getValueAt(row, 1).toString());
            txtEstado.setText(table.getValueAt(row, 2).toString());
            txtCultivo.setText(table.getValueAt(row, 3).toString());
            txtLugar.setText(table.getValueAt(row, 4).toString());
        }
    }
