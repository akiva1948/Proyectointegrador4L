...
public class cultivoTecnicoAd extends JFrame {
    ...
    private JTextField txtEspecie, txtNombres, txtVariedad, txtCiclo;

    private Connection conn;
    private JFrame parentMenu;
    private CultivoDAO cultivoDAO;

    public cultivoTecnicoAd(JFrame parentMenu, Connection conn) {
    ...
    private void initializeUI() {
        ...
        JLabel title = new JLabel("Catálogo de Cultivos");
        ...
        JLabel l1 = new JLabel("Detalle del Cultivo");
        l1.setFont(new Font("Segoe UI", Font.BOLD, 20));
        l1.setForeground(PRIMARY_COLOR);
        l1.setBounds(20, 20, 300, 30);
        left.add(l1);

        crearLabel(left, "Nombres:", 80);
        txtNombres = crearInput(left, 105);
        
        crearLabel(left, "Especie:", 160);
        txtEspecie = crearInput(left, 185);
        
        crearLabel(left, "Variedad:", 240);
        txtVariedad = crearInput(left, 265);
        
        crearLabel(left, "Ciclo:", 320);
        txtCiclo = crearInput(left, 345);
        ...
        tableModel = new DefaultTableModel(new String[]{"ID", "Nombres", "Especie", "Variedad", "Ciclo"}, 0){
    ...
    private void cargarDatos() {
        ...
        for(Cultivo c : cultivoDAO.obtenerTodosCultivos()) {
            tableModel.addRow(new Object[]{c.getIdCultivo(), c.getNombres(), c.getEspecie(), c.getVariedad(), c.getCiclo()});
        }
    }
    ...
    private void mostrarDetalle() {
        ...
        if(row != -1) {
            ...
            txtVariedad.setText(table.getValueAt(row, 3).toString());
            txtCiclo.setText(table.getValueAt(row, 4).toString());
        }
    }
