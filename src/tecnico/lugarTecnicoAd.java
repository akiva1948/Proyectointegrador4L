...
public class lugarTecnicoAd extends JFrame {
    ...
    private JTextField txtNombre, txtTel, txtCorreo, txtMun;

    private Connection conn;
    private JFrame parentMenu;
    private LugarProduccionDAO lugarDAO;

    public lugarTecnicoAd(JFrame parentMenu, Connection conn) {
    ...
    private void initializeUI() {
        setTitle("Consulta de Lugares de Producción");
        setSize(1366, 800);
        ...
        JLabel l1 = new JLabel("Detalles del Lugar");
        l1.setFont(new Font("Segoe UI", Font.BOLD, 20));
        l1.setForeground(PRIMARY_COLOR);
        l1.setBounds(20, 20, 300, 30);
        left.add(l1);

        crearLabel(left, "Nombre Lugar:", 80);
        txtNombre = crearInput(left, 105);
        
        crearLabel(left, "Teléfono:", 160);
        txtTel = crearInput(left, 185);
        
        crearLabel(left, "Correo:", 240);
        txtCorreo = crearInput(left, 265);
        
        crearLabel(left, "ID Municipio:", 320);
        txtMun = crearInput(left, 345);
        ...
        tableModel = new DefaultTableModel(new String[]{"ID", "Nombre", "Teléfono", "Correo", "ID Mun."}, 0){
    ...
    private void cargarDatos() {
        ...
        for(LugarProduccion l : lugarDAO.obtenerTodosLugares()) {
            tableModel.addRow(new Object[]{l.getIdLugarProduccion(), l.getNombreLugar(), l.getTelefono(), l.getCorreo(), l.getIdMunicipio()});
        }
    }
    ...
    private void mostrarDetalle() {
        ...
        if(row != -1) {
            ...
            txtTel.setText(table.getValueAt(row, 2).toString());
            txtCorreo.setText(table.getValueAt(row, 3).toString());
            txtMun.setText(table.getValueAt(row, 4).toString());
        }
    }
